...
if [ $MOSAIC_MODE == 0 ]; then
    ...
    gst-launch-1.0 v4l2src device=/dev/video-tevs-cam${CAM} io-mode=2 ! video/x-raw,width=640,height=480,format=UYVY ! tiovxldc out-block-width=128 ! video/x-raw,format=NV12 ! ${SINK} sync=false
    #gst-launch-1.0 v4l2src device=/dev/video-tevs-cam${CAM} io-mode=2 ! video/x-raw,width=640,height=480,format=UYVY ! glcolorconvert ! video/x-raw,format=NV12 ! ${SINK} sync=false
    ...
    #2x2 Mosaic, default
    gst-launch-1.0 \
    v4l2src device=/dev/video-tevs-cam3 io-mode=2 ! video/x-raw,width=640,height=480,format=UYVY ! tiovxldc out-block-width=128 ! video/x-raw,format=NV12 ! tiovxmultiscaler target=0 ! video/x-raw, width=640, height=400  ! mosaic.sink_3 \
    v4l2src device=/dev/video-tevs-cam2 io-mode=2 ! video/x-raw,width=640,height=480,format=UYVY ! tiovxldc out-block-width=128 ! video/x-raw,format=NV12 ! tiovxmultiscaler target=0 ! video/x-raw, width=640, height=400  ! mosaic.sink_2 \
    v4l2src device=/dev/video-tevs-cam1 io-mode=2 ! video/x-raw,width=640,height=480,format=UYVY ! tiovxldc out-block-width=128 ! video/x-raw,format=NV12 ! tiovxmultiscaler target=1 ! video/x-raw, width=640, height=400  ! mosaic.sink_1 \
    v4l2src device=/dev/video-tevs-cam0 io-mode=2 ! video/x-raw,width=640,height=480,format=UYVY ! tiovxldc out-block-width=128 ! video/x-raw,format=NV12 ! tiovxmultiscaler target=1 ! video/x-raw, width=640, height=400  ! mosaic.sink_0 \
